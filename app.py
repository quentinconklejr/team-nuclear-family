...
<style>
    .main-title {
        font-size: 1.9rem; font-weight: 700; color: #1a2e4a; margin-bottom: 0.15rem;
    }
    .subtitle {
        color: #5a6a7a; font-size: 0.95rem; margin-bottom: 1.25rem; line-height: 1.5;
    }
    .section-header {
        font-size: 1.05rem; font-weight: 600; color: #1a2e4a;
    ...
    .stSlider > label { font-weight: 600; font-size: 0.9rem; }
    div[data-testid="metric-container"] > div:first-child { font-size: 0.82rem; }
</style>
...
def build_geo_lookup(_geojson):
    ...
    for f in _geojson["features"]:
        p = f["properties"]
        state_abbr = p["state_abbr"] or _FIPS_TO_STATE.get(p.get("state_fips", ""), "")
        result[p["geoid"]] = {
...
# Merge pareto flags into main dataframe
df = candidates.merge(
...
# Percentile ranks (higher percentile = more of that variable than other counties)
_pct_cols = [
...
with st.sidebar:
    ...
    pga_filter = st.slider(
        ...
        format="%.2f g", label_visibility="collapsed",
    )
    ...
    st.markdown("**About**")
    st.caption(
        "Team Nuclear Family — IDSC Data Dive Spring 2026 🥇  \n"
...
m3.metric("Best MCDA Score", f"{filtered_df['mcda_score'].max():.3f}" if len(filtered_df) else "—")
...
# Layer 1: all counties in neutral gray (background)
fig.add_trace(go.Choroplethmap(
    geojson=geojson,
    locations=all_geoids,
    z=[0.0] * len(all_geoids),
    featureidkey="properties.geoid",
    colorscale=[[0, "#d6dae0"], [1, "#d6dae0"]],
    showscale=False,
    marker=dict(line=dict(width=0.3, color="#b0b8c4")),
    hoverinfo="skip",
    name="",
))
...
if len(filtered_df) > 0:
    _rank_str = filtered_df["rank"].apply(
        lambda x: f"#{int(x)}" if pd.notna(x) else "unranked"
    )
    _score_str = filtered_df["mcda_score"].apply(
    ...
    fig.add_trace(go.Choroplethmap(
        ...
        zmax=_score_max,
        colorbar=dict(
            title=dict(text="MCDA Score", font=dict(size=12, color="#1a2e4a")),
            thickness=14,
            len=0.55,
            x=1.01,
            tickformat=".2f",
        ),
        marker=dict(line=dict(width=0.5, color="#4a5568")),
...
if selection and hasattr(selection, "selection") and selection.selection.points:
    ...
    if loc and loc in filtered_geoids:
        ...
        # Clicked background (non-candidate or filtered-out county)
...
with table_col:
    ...
    if len(filtered_df) == 0:
        ...
        st.caption(
            ...
            "Click a row's county on the map to see its full profile."
        )
...
with detail_col:
    ...
    if selected_geoid and len(row_matches) > 0:
        ...
        badge = (
            ' <span class="pareto-badge">★ Pareto Tier 1</span>'
            if is_pareto else ""
        )
        st.markdown(
        ...
        if pd.notna(lake_d) and pd.notna(river_d):
            ...
            if nearest_water < 5:
                ...
                water_txt = f"Limited water — cooling infrastructure needed"
